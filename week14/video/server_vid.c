...
int main()
{
    ...
    if (fork() == 0)
    {
        ...
        struct sockaddr_in audio_client = client_addr; 
        audio_client.sin_port = htons(1235);         

        short audio_buf[2048]; 
        while (1)
        {
    ...
    else
    {
        ...
        while (1)
        {
            ...
            int in_linesize[1] = {640 * 2}; 
            
            sws_scale(sws_ctx, (const uint8_t *const *)in_data, in_linesize, 0, 480, frame->data, frame->linesize);
