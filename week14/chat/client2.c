...
int main(int argc, char *argv[]) {
    ...
    char pin[10];
    char pin_response[10];

    printf("Enter the 4-digit PIN given by the Server Admin: ");
    fgets(pin, 10, stdin);
    pin[strcspn(pin, "\r\n")] = 0;

    write(sockfd, pin, 4);

    bzero(pin_response, 10);
    read(sockfd, pin_response, 9);

    if (strncmp(pin_response, "OK_PIN", 6) != 0) {
        printf("Access Denied: Wrong PIN Code!\n");
        close(sockfd);
        exit(1);
    }
    ...
    if (pid < 0) {
        ...
        while (1) {
            ...
            if (strncmp(message, "bye", 3) == 0) {
                snprintf(send_buffer, sizeof(send_buffer), "[System] %s disconnected.", name);
                write(sockfd, send_buffer, strlen(send_buffer));
                printf("Disconnection requested. Exiting...\n");
                kill(pid, SIGKILL);
                break;
